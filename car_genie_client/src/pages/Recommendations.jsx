...
const Recommendations = () => {
    ...
    const currentVehicles = sortedRecommendations.slice(indexOfFirstVehicle, indexOfLastVehicle);
    
    // Change page
    const paginate = (pageNumber) => setCurrentPage(pageNumber);
    
    // Column definitions with headers and accessor functions - identical to FilterVehicles
    const columns = [
        { header: "ID", accessor: "vehicleId" },
        { header: "VIN", accessor: "vin" },
        { header: "Price", accessor: "price", format: (value) => `$${value?.toLocaleString() || 0}` },
    ...
    // Function to open a vehicle detail in a popup window - identical to FilterVehicles
    ...
    return (
        <div className="p-4 max-w-6xl mx-auto bg-black text-white min-h-screen">
            <h1 className="text-2xl font-bold mb-6">Recommend Vehicles</h1>
            <div className="bg-gray-900 p-4 rounded-lg mb-6">
                <div className="flex flex-col sm:flex-row gap-4">
                    <input
                        ...
                        className="p-2 border rounded bg-black text-white flex-grow"
                    />
                    <button
                        onClick={fetchRecommendations}
                        disabled={loading}
                        className="p-2 bg-white text-black rounded hover:bg-gray-300 disabled:opacity-50 whitespace-nowrap"
                    >
            ...
            {loading ? (
                ...
                <div className="mt-6">
                    <h2 className="text-lg font-semibold mb-4">Results: {recommendations.length} recommendations found</h2>
                    
                    {/* Responsive Table - Exact same format as FilterVehicles */}
                    ...
                    {/* Pagination - Exact same format as FilterVehicles */}
                    {recommendations.length > vehiclesPerPage && (
                        <div className="mt-6 flex flex-col items-center">
                            <div className="mb-2 text-sm">
                                Page <span className="font-bold">{currentPage}</span> of {Math.ceil(recommendations.length / vehiclesPerPage)}
                            </div>
                            <nav className="flex items-center">
                                ...
                                {/* Page numbers */}
                                {Array.from({ length: Math.ceil(recommendations.length / vehiclesPerPage) }).map((_, index) => {
                                    // Only show a window of 5 pages
                                    if ((index + 1 < currentPage - 2) || (index + 1 > currentPage + 2)) {
                                        // Only show first, last, and pages around current
                                        if (index !== 0 && index !== Math.ceil(recommendations.length / vehiclesPerPage) - 1) {
                                            return null;
                                        }
                                    }
                                    ...
                                    if (index === Math.ceil(recommendations.length / vehiclesPerPage) - 2 && currentPage < Math.ceil(recommendations.length / vehiclesPerPage) - 3) {
                                ...
                                {/* Next page button */}
                                <button
                                    onClick={() => paginate(currentPage < Math.ceil(recommendations.length / vehiclesPerPage) ? currentPage + 1 : currentPage)}
                                    disabled={currentPage === Math.ceil(recommendations.length / vehiclesPerPage)}
                                    className="px-3 py-1 border border-gray-700 bg-gray-800 disabled:opacity-50 mx-1"
                                >
                                    ›
                                </button>
                                ...
                                {/* Last page button */}
                                <button
                                    onClick={() => paginate(Math.ceil(recommendations.length / vehiclesPerPage))}
                                    disabled={currentPage === Math.ceil(recommendations.length / vehiclesPerPage)}
                                    className="px-3 py-1 border border-gray-700 bg-gray-800 disabled:opacity-50"
                                >
                                    »
            ...
            ) : hasSearched && !error ? ( // Only show "no results" message after a search
                <div className="text-center py-12 bg-gray-900 rounded-lg">
                    <p className="text-gray-300">No recommendations found. Try a different VIN.</p>
                </div>
            ) : null}
        </div>
    );
